    ...
    <div className="max-w-5xl mx-auto px-6 py-12">
      ...
      {sessions.length === 0 ? (
        <Card className="text-center py-16">
          ...
          <p className="text-slate-400 mb-6">Create your first collection session to get started</p>
          <Link href="/session/new">
...
function SessionCard({ session, onDelete }: { session: Session; onDelete: () => void }) {
  ...
  const getNextAction = () => {
    switch (session.status) {
      case 'created':
        return { href: `/session/${session.id}/documents`, label: 'Select Documents' };
      case 'link_generated':
      case 'in_progress':
      case 'submitted':
          ...
          {totalDocs > 0 && (
            <div className="mb-3">
              <div className="flex items-center justify-between text-sm mb-1">
                ...
                <span className="text-slate-300">{completedDocs}/{totalDocs} complete</span>
              </div>
